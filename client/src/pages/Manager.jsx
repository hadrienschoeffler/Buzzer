    ...
    return (
      <div style={styles.container}>
        <div style={styles.card}>
          ...
          <button style={styles.btnGhost} onClick={onLeave}>
            Quitter
          </button>
        </div>
      </div>
    ...
    <div style={styles.container}>
      <div style={styles.card}>
        <div style={styles.header}>
          <div>
            <h2 style={styles.title}>Salon</h2>

            <button style={styles.codeButton} onClick={handleCopy} title="Copier le code">
              {room?.code}
            </button>

            {copied && <p style={styles.copied}>Code copié</p>}
          </div>
        ...
        <div
          style={{
            ...styles.buzzerZone,
            background: buzzed ? '#e6394622' : '#0f0f1a',
            borderColor: buzzed ? '#e63946' : '#333',
          }}
        >
          {buzzed ? (
            <>
              <span style={styles.buzzLabel}>A buzzé</span>
              <span style={styles.buzzName}>{buzzed}</span>
            </>
          ) : (
        ...
        <div>
          <h3 style={styles.sectionTitle}>
            Participants ({room?.participants?.filter((p) => p.online).length || 0} /{' '}
            ...
            room?.participants?.map((p) => (
              <div key={p.pseudo} style={{ ...styles.playerRow, opacity: p.online ? 1 : 0.4 }}>
                <span style={{ flex: 1 }}>
                  {p.pseudo}
                  {!p.online && (
                    <span style={{ fontSize: '0.75rem', color: '#e67e22', marginLeft: '0.4rem' }}>
                      • absent
                    </span>
                  )}
                </span>
